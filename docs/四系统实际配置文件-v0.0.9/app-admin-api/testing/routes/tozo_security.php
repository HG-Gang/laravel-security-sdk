...
/**
 * 后台管理 API（app-admin-api）测试环境入站安全路由
 *
 * 文件功能：
...
// 来自 tozo-app-api 的入站请求：先验签，再生成响应签名。
Route::middleware(['tozo.inbound', 'tozo.response'])
    ->defaults('tozo_profile', 'app_admin_api_inbound_from_tozo_app_api')
    ->post('/api/internal/tozo-security/from-tozo-app-api/health', [TozoSecurityController::class, 'handle']);

// 来自 pmc-api 的入站请求：先验签，再生成响应签名。
Route::middleware(['tozo.inbound', 'tozo.response'])
    ->defaults('tozo_profile', 'app_admin_api_inbound_from_pmc_api')
    ->post('/api/internal/tozo-security/from-pmc-api/health', [TozoSecurityController::class, 'handle']);

// 来自 pos-api 的入站请求：先验签，再生成响应签名。
Route::middleware(['tozo.inbound', 'tozo.response'])
    ->defaults('tozo_profile', 'app_admin_api_inbound_from_pos_api')
